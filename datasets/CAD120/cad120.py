...
class CAD120(torch.utils.data.Dataset):
    ...
    def __init__(self, feature_data_path, sequence_ids):
        if not self.__class__.features:
            self.__class__.features = pickle.load(open(feature_data_path, 'rb'),encoding='bytes') # here used input as bytes, we should convert them to be str later
        self.data = list()
        self.sequence_ids = list()
        for sequence_id, sequence_features in self.__class__.features.items():
            if sequence_id not in sequence_ids: # used to be in, but obviously it is not true
                str_features = []
                for i in range(len(sequence_features)): # change byte to string
                    str_features.append({key.decode('utf-8'): value for key, value in sequence_features[i].items()})
                self.data.extend(str_features)
                self.sequence_ids.extend([sequence_id.decode('utf-8') for _ in range(len(sequence_features))])
