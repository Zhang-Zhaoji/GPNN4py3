...
from datasets.utils import load_best_checkpoint,save_checkpoint
...
action_class_num = len(dts.vcoco_metadata.action_classes)
roles_num = len(dts.vcoco_metadata.roles)
...
def train(args, train_loader, model, mse_loss, multi_label_loss, optimizer, epoch, vcocoeval, logger):
    ...
    for i, (edge_features, node_features, adj_mat, node_labels, node_roles, boxes, img_ids, img_names, human_nums, obj_nums, classes) in enumerate(train_loader):
        ...
        # TODO here we have to cancel part of code due to error
...
def parse_arguments():
    ...
    feature_type = 'vcoco_features'#'resnet'
