...
def main(args):
    ...
    # Load data
    training_set, valid_set, testing_set, train_loader, valid_loader, test_loader = dts.utils.get_cad_data(args)
...
def train(train_loader, model, criterion, optimizer, epoch, logger, args=None):
    ...
    for i, (edge_features, node_features, adj_mat, node_labels, sequence_ids, node_nums) in enumerate(train_loader):
        ...
        # Log
        loss_value = train_loss.item()  # 保存train_loss的值，用于记录和打印
        losses.update(loss_value, edge_features.size(0))  # 使用保存的值更新losses
...
def validate(val_loader, model, criterion, logger=None, args=None, test=False):
    ...
    for i, (edge_features, node_features, adj_mat, node_labels, sequence_ids, node_nums) in enumerate(val_loader):
        ...
        # Logs
        # it just raise an error out of no reason
        losses.update(criterion(pred_node_labels, node_labels).items(), edge_features.size(0))
    ...
    if args.visualize:
        ...
        confusion_matrix = sklearn.metrics.confusion_matrix(subact_ground_truth, subact_predictions,
                                                            labels=range(len(dts.cad_metadata.subactivities)))
        utils.plot_confusion_matrix(confusion_matrix, dts.cad_metadata.subactivities, normalize=True, title='',
        ...
        confusion_matrix = sklearn.metrics.confusion_matrix(affordance_ground_truth, affordance_predictions,
                                                            labels=range(len(dts.cad_metadata.affordances)))
        utils.plot_confusion_matrix(confusion_matrix, dts.cad_metadata.affordances, normalize=True, title='',
