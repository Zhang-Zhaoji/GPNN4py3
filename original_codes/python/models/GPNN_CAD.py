...
class GPNN_CAD(torch.nn.Module):
    ...
    def forward(self, edge_features, node_features, adj_mat, node_labels, args):
        ...
        pred_node_labels = torch.zeros_like(node_labels)# torch.tensor(torch.zeros(node_labels.size()))
        if args.cuda:
        ...
        hidden_node_states = [node_features.clone().detach() for passing_round in range(self.propagate_layers+1)]
        hidden_edge_states = [edge_features.clone().detach()for passing_round in range(self.propagate_layers+1)]
        ...
        for passing_round in range(self.propagate_layers):
            ...
            for i_node in range(node_features.size()[2]):
                ...
                hidden_edge_states[passing_round+1][:, :, :, i_node] = m_v.clone().detach()
                m_v = torch.sum(m_v, 2)
                if i_node == 0:
