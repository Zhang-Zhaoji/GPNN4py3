...
import datasets
import datasets.utils as uts
# import visualization_utils as vu


## we have to keep continue using api of dataset I guess
def get_cad_data(args, prediction=False):
    ...
    train_loader = torch.utils.data.DataLoader(training_set, collate_fn=uts.collate_fn_cad,
                                               batch_size=args.batch_size,
                                               num_workers=args.num_workers, pin_memory=True)
    valid_loader = torch.utils.data.DataLoader(valid_set, collate_fn=uts.collate_fn_cad,
                                               batch_size=args.batch_size,
                                               num_workers=args.num_workers, pin_memory=True)
    test_loader = torch.utils.data.DataLoader(testing_set, collate_fn=uts.collate_fn_cad,
                                              batch_size=args.batch_size,
                                              num_workers=args.num_workers, pin_memory=True)
...
def get_hico_data(args):
    ...
    root = os.path.join(args.hico_data_root, 'processed', 'hico_data_background_49')
    ...
    train_loader = torch.utils.data.DataLoader(training_set, collate_fn=uts.collate_fn_hico,
                                               batch_size=args.batch_size, shuffle=True,
                                               num_workers=args.num_workers, pin_memory=True)
    valid_loader = torch.utils.data.DataLoader(valid_set, collate_fn=uts.collate_fn_hico,
                                               batch_size=args.batch_size,
                                               num_workers=args.num_workers, pin_memory=True)
    test_loader = torch.utils.data.DataLoader(testing_set, collate_fn=uts.collate_fn_hico,
                                               batch_size=args.batch_size,
                                               num_workers=args.num_workers, pin_memory=True)
    print('Dataset sizes: {} training, {} validation, {} testing.'.format(len(train_loader), len(valid_loader), len(test_loader)))
    return training_set, valid_set, testing_set, train_loader, valid_loader, test_loader
...
def get_vcoco_data(args):
    ...
    root = os.path.join(args.vcoco_data_root, 'processed', args.feature_type)
    training_set = datasets.VCOCO(root, 'train')
    valid_set = datasets.VCOCO(root, 'val')
    testing_set = datasets.VCOCO(root, 'test')
    train_loader = torch.utils.data.DataLoader(training_set, collate_fn=uts.collate_fn_vcoco,
                                               batch_size=args.batch_size, shuffle=True,
                                               num_workers=args.num_workers, pin_memory=True)
    valid_loader = torch.utils.data.DataLoader(valid_set, collate_fn=uts.collate_fn_vcoco,
                                               batch_size=args.batch_size,
                                               num_workers=args.num_workers, pin_memory=True)
    test_loader = torch.utils.data.DataLoader(testing_set, collate_fn=uts.collate_fn_vcoco,
                                               batch_size=args.batch_size,
                                               num_workers=args.num_workers, pin_memory=True)
    print('Dataset sizes: {} training, {} validation, {} testing.'.format(len(train_loader), len(valid_loader), len(test_loader)))
    return training_set, valid_set, testing_set, train_loader, valid_loader, test_loader
